...
const Cart = ({ 
  ...
  const getCartItemCount = () => {
    return cartItems.length; // Now just count the items since each is quantity 1
  };
  ...
  const getCartTotalWeight = () => {
    return cartItems
      .reduce((total, item) => {
        ...
        return total + weightNum; // No multiplication since quantity is always 1
      }, 0)
      .toFixed(1);
  };
  ...
  const getCartTotalPrice = () => {
    return cartItems.reduce((total, item) => {
      ...
      return total + price; // No multiplication since quantity is always 1
    }, 0);
  };
  ...
  useEffect(() => {
   ...
   const confirmOrder = async () => {
    try {
      if (onConfirmOrder) {
        // Use the parent's confirmOrder function which handles orders table
...
const CartItem = ({ item, onRemove, removing }) => {
  ...
  const calculateItemTotal = () => {
    ...
    return price.toLocaleString(); // No quantity multiplication
  };
